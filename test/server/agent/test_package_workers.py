...
def test_spawn_package_worker_limits_concurrency(monkeypatch, tmp_path: Path) -> None:
...
def test_spawn_package_worker_requires_real_package_project(tmp_path: Path) -> None:
    ctx = AppContext(workspace_paths=[tmp_path])
    setattr(ctx, "agent_session_id", "session-1")
    setattr(ctx, "agent_run_id", "run-1")
