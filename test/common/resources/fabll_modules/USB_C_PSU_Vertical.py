...
class USB_C_PSU_Vertical(fabll.Module):
    ...
    def __preinit__(self):
        ...
        self.gnd_capacitor.max_voltage.constrain_subset(
            fabll.Range.from_center_rel(16 * P.V, 0.05)
        )
        self.gnd_resistor.resistance.constrain_subset(
            fabll.Range.from_center_rel(1 * P.Mohm, 0.05)
        )
        for res in self.configuration_resistors:
            res.resistance.constrain_subset(
                fabll.Range.from_center_rel(5.1 * P.kohm, 0.05)
            )
        ...
        vbus_fuse.trip_current.constrain_subset(
            fabll.Range.from_center_rel(1 * P.A, 0.05)
        )
