...
def test_edge_type_consistency():
    ...
    assert fbrk.EdgeInterfaceConnection.is_instance(edge=normal)
    assert fbrk.EdgeInterfaceConnection.is_instance(edge=shallow)
...
def test_visit_connected_edges_callback_receives_python_objects():
    ...
    for entry in collected:
        assert fbrk.EdgeInterfaceConnection.is_instance(edge=entry.edge())
...
def test_get_connected_path_objects_cleanup():
    ...
    assert (
        ...
        .node()
        .is_same(other=path_to_n3_v2.get_start_node().node())
    )
    assert (
        path_to_n3_v1.get_end_node()
        .node()
        .is_same(other=path_to_n3_v2.get_end_node().node())
    )
