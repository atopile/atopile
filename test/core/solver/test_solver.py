...
def test_relevance_set_resume_subset():
    """
    Resume from a previous solve with a shrinking relevance set.
    Solve for {A, B}, then resume for {A} only. Assert the bootstrap stage
    is named 'bootstrap_relevant_resume', the graph is fresh, and A's bounds
    are correct.
    """
    E = BoundExpressions()
    A, B = _create_letters(E, 2, units=E.U.Ohm)
    ...
    E.is_subset(A_op, E.lit_op_range(((1, E.U.Ohm), (10, E.U.Ohm))), assert_=True)
    E.is_subset(B_op, E.lit_op_range(((20, E.U.Ohm), (30, E.U.Ohm))), assert_=True)

    # Initial solve for both A and B
    solver = Solver()
    solver.simplify(E.g, E.tg, terminal=False, relevant=[A_op, B_op])

    # Resume for A only (shrinking relevance)
    solver2 = Solver.from_initial_state(solver.state.compressed())
    state2 = solver2.simplify(E.g, E.tg, terminal=False, relevant=[A_op])

    # The first bootstrap stage should use resume
    assert state2.data.mutation_map.first_stage.algorithm == (
        "bootstrap_relevant_resume"
    )

    # Fresh graph — not the same object as the original G_out
    assert (
        state2.data.mutation_map.first_stage.G_out.get_self_node().node().get_uuid()
        != solver.state.data.mutation_map.G_out.get_self_node().node().get_uuid()
    )

    # A should still have correct bounds
    assert _extract_and_check(
        A_op,
        state2.data.mutation_map,
        E.lit_op_range(((1, E.U.Ohm), (10, E.U.Ohm))),
    )


def test_relevance_set_resume_with_new_predicates():
    """
    Resume from a previous solve after adding new constraints to the source graph
    (simulating attach). The new constraint on A should be visible when resuming
    for {A, B}.
    """
    E = BoundExpressions()
    A, B = _create_letters(E, 2, units=E.U.Ohm)
    ...
    # Create constraints
    E.is_subset(A_op, E.lit_op_range(((1, E.U.Ohm), (100, E.U.Ohm))), assert_=True)
    E.is_subset(B_op, E.lit_op_range(((1, E.U.Ohm), (100, E.U.Ohm))), assert_=True)

    # Initial solve
    solver = Solver()
    solver.simplify(E.g, E.tg, terminal=False, relevant=[A_op, B_op])

    # Add a new tighter constraint on A (simulating attach after pick)
    E.is_superset(A_op, E.lit_op_range(((5, E.U.Ohm), (10, E.U.Ohm))), assert_=True)

    # Resume for both A and B
    solver2 = Solver.from_initial_state(solver.state.compressed())
    state2 = solver2.simplify(E.g, E.tg, terminal=False, relevant=[A_op, B_op])

    # A should have the superset constraint (from first solve, seeded) and the new
    # IsSuperset constraint. The extracted superset should be at most {1..100}.
    assert _extract_and_check(
        A_op,
        state2.data.mutation_map,
        E.lit_op_range(((1, E.U.Ohm), (100, E.U.Ohm))),
    )


def test_relevance_set_resume_full_solver_integration():
    """
    End-to-end: use Solver.simplify twice with shrinking relevance and a new
    attach-style constraint between calls. Second call should produce correct
    results.
    """
    E = BoundExpressions()
    A, B, C = _create_letters(E, 3, units=E.U.V)
    A_op, B_op, C_op = A.as_operand.get(), B.as_operand.get(), C.as_operand.get()

    # A, B, C all constrained
    E.is_subset(A_op, E.lit_op_range(((0, E.U.V), (10, E.U.V))), assert_=True)
    E.is_subset(B_op, E.lit_op_range(((0, E.U.V), (10, E.U.V))), assert_=True)
    E.is_subset(C_op, E.lit_op_range(((0, E.U.V), (10, E.U.V))), assert_=True)

    # B = A + C (shared expression linking them)
    E.is_(B_op, E.add(A_op, C_op), assert_=True)

    # First solve: all three relevant
    solver = Solver()
    solver.simplify(E.g, E.tg, terminal=False, relevant=[A_op, B_op, C_op])

    # "Pick" A to [2, 3] by adding an IsSuperset constraint
    E.is_superset(A_op, E.lit_op_range(((2, E.U.V), (3, E.U.V))), assert_=True)

    # Second solve: resume with all three relevant
    solver2 = Solver.from_initial_state(solver.state.compressed())
    state2 = solver2.simplify(E.g, E.tg, terminal=False, relevant=[A_op, B_op, C_op])

    # A should have a superset bound (at most [0, 10] from initial constraint)
    assert _extract_and_check(
        A_op,
        state2.data.mutation_map,
        E.lit_op_range(((0, E.U.V), (10, E.U.V))),
    )

    # B should also have a superset bound
    assert _extract_and_check(
        B_op,
        state2.data.mutation_map,
        E.lit_op_range(((0, E.U.V), (10, E.U.V))),
