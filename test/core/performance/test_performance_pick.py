...
def test_performance_pick_real_module(
    module_type: Callable[[], Module],  # = F.RP2040_ReferenceDesign,
):
    timings = Times()
...
def _pretty_timings(timings: Times):
    ...
    return timings.to_str(force_unit="ms")
...
def test_performance_pick_rc_formulas():
    ...
    for i in range(_GROUPS):
        picked_values = {
            m.get_full_name(): str(m.resistance.try_get_literal())
            for m in app.res[i::_GROUPS]  # + [app.alias_res[i]]
        }
        logger.info(f"Picked values Group {i}: {indented_container(picked_values)}")
