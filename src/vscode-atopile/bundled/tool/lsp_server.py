...
_error_accumulators: dict[Path, faebryk.libs.exceptions.errors.ExceptionAccumulator] = (
    defaultdict(faebryk.libs.exceptions.errors.ExceptionAccumulator)
)
...
def _index_class_defs_by_line(file: Path):
    ...
    except* faebryk.libs.exceptions.errors._BaseUserException as ex:
...
def publish_errors(uri: str):
    ...
    for error in _error_accumulators[file].errors:
        if (
            isinstance(error, faebryk.libs.exceptions.errors._BaseUserException)
            and error.src_path
            and error.src_col
            and error.src_line
        ):
...
def completions(params: Optional[lsp.CompletionParams]) -> lsp.CompletionList:
    ...
    except (KeyError, faebryk.libs.exceptions.errors.UserException):
        pass
    else:
...
def hover_definition(params: lsp.HoverParams) -> Optional[lsp.Hover]:
    ...
    try:
        ...
        instance = atopile.front_end.lofty.get_instance(instance_addr)
    except (KeyError, faebryk.libs.exceptions.errors.UserException, AttributeError):
        pass
    else:
