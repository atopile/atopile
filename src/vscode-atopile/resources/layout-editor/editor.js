...
function isAnnotationCopperLayerHidden(layerName, categories, layerById) {
  if (!layerById)
    return false;
  const kind = (layerById.get(layerName)?.kind ?? "").toLowerCase();
  return kind === "cu" && categories.has(layerName + ":pads");
}
function buildPadAnnotationGeometry(fp, hiddenLayers, hiddenCategories, layerById) {
  const hidden = hiddenLayers ?? /* @__PURE__ */ new Set();
  const categories = hiddenCategories ?? /* @__PURE__ */ new Set();
  ...
  for (const annotation of fp.pad_names) {
    ...
    for (const layerName of annotation.layer_ids) {
      if (hidden.has(layerName))
        continue;
      if (isAnnotationCopperLayerHidden(layerName, categories, layerById))
        continue;
...
function paintAll(renderer, model, hiddenLayers, hiddenCategories) {
  const hidden = hiddenLayers ?? /* @__PURE__ */ new Set();
  const categories = hiddenCategories ?? /* @__PURE__ */ new Set();
...
function paintZones(renderer, model, hidden, categories, layerById) {
  for (const zone of model.zones) {
    ...
    for (const filled of sortedFilledPolygons) {
      if (hidden.has(filled.layer))
        continue;
      if (categories.has(filled.layer + ":zones"))
        continue;
    ...
    if (shouldDrawFillFromOutline) {
      ...
      for (const layerName of zoneLayers) {
        if (!layerName || hidden.has(layerName))
          continue;
        if (categories.has(layerName + ":zones"))
          continue;
...
function paintTracks(renderer, model, hidden, categories, layerById) {
...
function isPadCategoryHidden(pad, categories, layerById) {
...
function paintFootprint(renderer, fp, hidden, categories, layerById) {
  ...
  if (fp.pads.length > 0) {
    const anyVisible = fp.pads.some(
      (pad) => pad.layers.some((layerName) => !hidden.has(layerName)) && !isPadCategoryHidden(pad, categories, layerById)
    );
    if (anyVisible) {
...
function paintPadAnnotations(renderer, fp, hidden, categories, layerById) {
  if (fp.pads.length === 0)
    return;
...
function renderTextOverlay(ctx, model, camera, hiddenLayers, layerById, hiddenCategories) {
...
var Editor = class {
  ...
  hiddenCategories = /* @__PURE__ */ new Set();
