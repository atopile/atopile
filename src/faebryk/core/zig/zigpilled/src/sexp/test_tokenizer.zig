...
test "tokenize string literal" {
    ...
    const tokens = try tokenizer.tokenize(std.testing.allocator, input);
    defer std.testing.allocator.free(tokens);
    
    try std.testing.expectEqual(@as(usize, 1), tokens.len);
    try std.testing.expectEqual(tokenizer.TokenType.string, tokens[0].type);
...
test "tokenize symbol" {
    ...
    try std.testing.expectEqual(@as(usize, 1), tokens.len);
    try std.testing.expectEqual(tokenizer.TokenType.symbol, tokens[0].type);
...
test "tokenize number" {
    ...
    try std.testing.expectEqual(@as(usize, 1), tokens.len);
    try std.testing.expectEqual(tokenizer.TokenType.number, tokens[0].type);
...
test "tokenize parentheses" {
    ...
    try std.testing.expectEqual(tokenizer.TokenType.lparen, tokens[0].type);
    try std.testing.expectEqual(tokenizer.TokenType.rparen, tokens[1].type);
}
...
test "tokenize simple s-expression" {
    ...
    try std.testing.expectEqual(tokenizer.TokenType.lparen, tokens[0].type);
    try std.testing.expectEqual(tokenizer.TokenType.symbol, tokens[1].type);
    try std.testing.expectEqualStrings("hello", tokens[1].value);
    try std.testing.expectEqual(tokenizer.TokenType.symbol, tokens[2].type);
    try std.testing.expectEqualStrings("world", tokens[2].value);
    try std.testing.expectEqual(tokenizer.TokenType.rparen, tokens[3].type);
}
...
test "tokenize nested s-expression" {
    ...
    try std.testing.expectEqual(tokenizer.TokenType.lparen, tokens[0].type);
    try std.testing.expectEqual(tokenizer.TokenType.symbol, tokens[1].type);
    try std.testing.expectEqual(tokenizer.TokenType.lparen, tokens[2].type);
    try std.testing.expectEqual(tokenizer.TokenType.symbol, tokens[3].type);
    try std.testing.expectEqual(tokenizer.TokenType.number, tokens[4].type);
    try std.testing.expectEqual(tokenizer.TokenType.rparen, tokens[5].type);
    try std.testing.expectEqual(tokenizer.TokenType.rparen, tokens[6].type);
}
...
test "tokenize kicad-style expression" {
    ...
    try std.testing.expectEqual(tokenizer.TokenType.string, tokens[2].type);
    try std.testing.expectEqualStrings("Reference", tokens[2].value);
    try std.testing.expectEqual(tokenizer.TokenType.string, tokens[3].type);
    try std.testing.expectEqualStrings("C1", tokens[3].value);
    try std.testing.expectEqual(tokenizer.TokenType.lparen, tokens[4].type);
    try std.testing.expectEqual(tokenizer.TokenType.symbol, tokens[5].type);
    try std.testing.expectEqualStrings("at", tokens[5].value);
    try std.testing.expectEqual(tokenizer.TokenType.number, tokens[6].type);
    try std.testing.expectEqualStrings("0", tokens[6].value);
    try std.testing.expectEqual(tokenizer.TokenType.number, tokens[7].type);
    try std.testing.expectEqualStrings("-4", tokens[7].value);
    try std.testing.expectEqual(tokenizer.TokenType.number, tokens[8].type);
    try std.testing.expectEqualStrings("0", tokens[8].value);
    try std.testing.expectEqual(tokenizer.TokenType.rparen, tokens[9].type);
    try std.testing.expectEqual(tokenizer.TokenType.rparen, tokens[10].type);
}
...
test "tokenize multiline expression" {
    ...
    try std.testing.expectEqual(tokenizer.TokenType.lparen, tokens[6].type);
    try std.testing.expectEqual(tokenizer.TokenType.symbol, tokens[7].type);
    try std.testing.expectEqualStrings("legacy_teardrops", tokens[7].value);
    try std.testing.expectEqual(tokenizer.TokenType.symbol, tokens[8].type);
...
test "tokenize ato.kicad_pcb excerpt" {
    ...
    try std.testing.expectEqual(tokenizer.TokenType.string, tokens[8].type);
    try std.testing.expectEqualStrings("atopile", tokens[8].value);
    try std.testing.expectEqual(tokenizer.TokenType.rparen, tokens[9].type);
    
    try std.testing.expectEqual(tokenizer.TokenType.lparen, tokens[10].type);
    try std.testing.expectEqual(tokenizer.TokenType.symbol, tokens[11].type);
    try std.testing.expectEqualStrings("generator_version", tokens[11].value);
    try std.testing.expectEqual(tokenizer.TokenType.string, tokens[12].type);
    try std.testing.expectEqualStrings("0.3.15-dev0", tokens[12].value);
    try std.testing.expectEqual(tokenizer.TokenType.rparen, tokens[13].type);
    try std.testing.expectEqual(tokenizer.TokenType.rparen, tokens[14].type);
}