...
pub const IsUnit = struct {
    ...
    pub fn get_base_units(self: IsUnit) []const BaseUnit {
        // Get the parent node (the unit)
        const parent_node = EdgeComposition.get_parent_node_of(self.node).?;
        // Visit children of the parent node
        const children = EdgeComposition.visit_children_of_type(
            parent_node,
            BaseUnit,
            &ctx,
            return_first(BaseUnit).visit,
        );
    }
};

pub const BaseUnit = struct {
    node: BoundNodeReference,

    trait.mark_as_trait(IsBaseUnit);

    const base_unit_identifier = "base_unit";
    pub fn of(node: BoundNodeReference) BaseUnit {
        return BaseUnit{ .node = node };
    }
};
...
test "IsUnit.get_base_units" {
    ...
    const unit = g.create_and_insert_node();
    // Add an 'is base unit' trait to the unit
    _ = Trait.add_trait_to(unit, IsBaseUnit);
}
