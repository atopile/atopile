...
class ComponentQuery(_RaiseType): ...


class Component(_RaiseType): ...


class MappingParameterDB(_RaiseType): ...
...
if False:
    ...
    class Manufacturers(Model):
        ...
        async def get_ids(self, manufacturer: str) -> list[int]:
            ...
            """
            manufacturer_ids = await self.filter(name__icontains=manufacturer).values(
                "id"
            )
            if len(manufacturer_ids) < 1:
                raise LookupError(
                    f"Could not find a match for manufacturer {manufacturer}"
                )
        ...
        def get_parameter(self, m: MappingParameterDB) -> Parameter:
            ...
            attr_key = next(
                (
                    ...
                    if k in self.extra.get("attributes", "")
                ),
                None,
            )
        ...
        def attach(
            ...
            if not allow_TBD and any(isinstance(p, TBD_ParseError) for p in params):
                params_str = indent(
                    "\n"
                    + "\n".join(
                        repr(p) for p in params if isinstance(p, TBD_ParseError)
                    ),
                    " " * 4,
                )
        ...
        def download(
            ...
            def get_number_of_volumes(zip_path):
                with open(zip_path, "rb") as f:
                    ...
                    if len(
                        end_of_central_dir
                    ) != 22 or not end_of_central_dir.startswith(b"PK\x05\x06"):
