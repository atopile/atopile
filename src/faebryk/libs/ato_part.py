...
class AtoPart:
    ...
    path: Path
    mfn: tuple[str, str]
    fp: kicad.footprint.FootprintFile
    symbol: kicad.symbol.SymbolFile
    ...
    def fp_path(self) -> Path:
        return self.path / f"{kicad.fp_get_base_name(self.fp.footprint)}.kicad_mod"
    ...
    def __post_init__(self):
        ...
        if self.model:
            ...
            self.fp.footprint.models[0].path = rel_path.as_posix()
    ...
    def dump(self):
        ...
        if self.datasheet:
            # TODO: Fix me
            ato_builder.add_trait("has_datasheet", datasheet=self.datasheet)
    ...
    def load(cls, path: Path) -> Self:
        ...
        datasheet = (
            t.get_datasheet() if (t := ato.try_get_trait(F.has_datasheet)) else None
        )
        ...
        fp = kicad.loads(kicad.footprint.FootprintFile, path / atomic_trait._footprint)
        symbol = kicad.loads(kicad.symbol.SymbolFile, path / atomic_trait._symbol)
        model = (
        ...
        if auto_generated_trait is not None:
            assert auto_generated_trait.system is not None
            assert auto_generated_trait.date is not None

            auto_generated = AtoPart.AutoGenerated(
                system=auto_generated_trait.system,
                date=datetime.datetime.fromisoformat(auto_generated_trait._date),
                source=auto_generated_trait.source,
                checksum=auto_generated_trait.checksum,
            )
