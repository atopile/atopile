...
_e_series_cache: dict[tuple[Ranges, int], L.Ranges] = {}


def e_series_intersect(
    value_set: Range[QuantityT] | Ranges[QuantityT],
    e_series: E_SERIES = E_SERIES_VALUES.E_ALL,
) -> L.Ranges[QuantityT]:
    if isinstance(value_set, Range):
        value_set = Ranges(value_set)

    if (value_set, id(e_series)) in _e_series_cache:
        return _e_series_cache[(value_set, id(e_series))]
    ...
    _e_series_cache[(value_set, id(e_series))] = out
    return out
