from faebryk.libs.picker.jlcpcb.jlcpcb import _RaiseType


class E_SERIES_VALUES(_RaiseType): ...


def e_series_intersect(*args, **kwargs):
    raise NotImplementedError()


def e_series_ratio(*args, **kwargs):
    raise NotImplementedError()


def e_series_discretize_to_nearest(*args, **kwargs):
    raise NotImplementedError()
...
if False:
    ...
    def e_series_discretize_to_nearest(
        ...
        target = (
            value.value if isinstance(value, F.Constant) else sum(value.as_tuple()) / 2
        )
    ...
    def e_series_ratio(
        ...
        rl_values = (
            e_series_intersect(rl, e_values) if isinstance(rl, F.Range) else None
        )
