...
class FootprintLibraryNotInTable(Exception):
...
class FootprintLibraryTableNotFound(Exception):
    def __init__(self, *args: object, lib_table_path: Path) -> None:
        super().__init__(*args)
        self.lib_table_path = lib_table_path


class FootprintLibraryTableCorrupted(Exception):
    def __init__(self, *args: object, lib_table_path: Path) -> None:
        super().__init__(*args)
        self.lib_table_path = lib_table_path


class FootprintLibraryTableInsertErrorFootprintFileNotFound(Exception):
    def __init__(self, *args: object, lib_table_path: Path, fp_path: Path) -> None:
        super().__init__(*args)
        self.lib_table_path = lib_table_path
        self.fp_path = fp_path


class FootprintLibraryTableInsertErrorDuplicateLibrary(Exception):
    def __init__(
        self,
        *args: object,
        lib_table: C_kicad_fp_lib_table_file,
        lib_name: str,
        fp_uri: Path,
    ) -> None:
        msg = (
            f"Footprint library {lib_name} already exists at different"
            f" location: {fp_uri} != {lib_table.fp_lib_table.libs[lib_name].uri}. "
            "Manual ingestion required."
        )
        logger.error(msg)
        super().__init__(*args)
        self.lib_table = lib_table
        self.lib_name = lib_name
        self.fp_uri = fp_uri


class PartWithoutProject(Exception):
    def __init__(self, *args: object, part_path: Path) -> None:
        super().__init__(*args)
        self.part_path = part_path


class PartWithoutProjectConfigNotFound(PartWithoutProject):
    pass


class PartWithoutProjectConfigInvalid(PartWithoutProject):
    def __init__(self, *args: object, part_path: Path, config_path: Path) -> None:
        super().__init__(*args, part_path=part_path)
        self.config_path = config_path


class FootprintNotFound(Exception):
    def __init__(self, *args: object, fp_id: str) -> None:
        super().__init__(*args)
        self.fp_id = fp_id
    ...
    class Library:
        ...
        def __insert_fp_lib(
            ...
            if not fppath.resolve().exists():
                raise FootprintLibraryTableInsertErrorFootprintFileNotFound(
                    lib_table_path=fp_table_path, fp_path=fppath
                )
            ...
            if lib_name in fp_table.fp_lib_table.libs:
                if fp_table.fp_lib_table.libs[lib_name].uri != fpuri:
                    raise FootprintLibraryTableInsertErrorDuplicateLibrary(
                        lib_table=fp_table,
                        lib_name=lib_name,
                        fp_uri=fpuri,
                    )
        ...
        def get_part_from_footprint_identifier(
            ...
            def _find_footprint(
                ...
                err_accumulator = accumulate(
                    FootprintLibraryNotInTable,
                    FootprintLibraryTableNotFound,
                    FootprintLibraryTableCorrupted,
                )
                ...
                for lib_table_path in lib_tables:
                    with err_accumulator.collect():
                        try:
                            ...
                            raise FootprintLibraryTableNotFound(
                                lib_table_path=lib_table_path
                            ) from ex
                        except Exception as ex:
                            raise FootprintLibraryTableCorrupted(
                                lib_table_path=lib_table_path
                            ) from ex
                        ...
                        try:
                            ...
                            raise FootprintLibraryNotInTable(
            ...
            try:
                ...
                if (
                    ...
                    except FootprintLibraryTableInsertErrorFootprintFileNotFound:
                        raise
        ...
        @staticmethod
        def _get_project_from_part_path(part_path: Path) -> ProjectConfig:
            # TODO: hate this
            manifest_path = _find_project_config_file(part_path)
            if manifest_path is None:
                raise PartWithoutProjectConfigNotFound(part_path=part_path)
            cfg = ProjectConfig.from_path(manifest_path.parent)
            if not cfg:
                raise PartWithoutProjectConfigInvalid(
                    part_path=part_path, config_path=manifest_path
                )
            return cfg
        ...
        def _fix_package_footprint_model(
            ...
            package_project_config = self._get_project_from_part_path(fp_path)
        ...
        def get_footprint_from_identifier(
            ...
            try:
                part_path = self.get_part_from_footprint_identifier(
                    identifier, component
                )
            except PartWithoutProject as ex:
                raise FootprintNotFound(fp_id=identifier) from ex
