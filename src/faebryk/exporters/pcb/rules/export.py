...
def _mm_str(value_mm: float) -> str:
    """Format millimeter value for DRU file."""
    return f"{value_mm:.2f}mm"


def _emit_rule(name: str, clauses: list[str]) -> str:
    """Emit a rule as S-expression string."""
    inner = "\n  ".join(clauses)
    return f"(rule {name}\n  {inner}\n)"


def _write_rules_file(rules: list[str], path: Path) -> None:
    """Write rules to DRU file."""
...
def export_rules(app: Module, solver: Solver) -> None:
    ...
    if not net_currents:
        ...
        _write_rules_file(rules, rules_file)
        return
    ...
    for zdiff, dps in dp_by_impedance.items():
        ...
        for i, (stack_idx, t_mm) in enumerate(copper_layers):
            ...
            if dps:
                ...
                if net_conditions:
                    condition = f'(condition "{" || ".join(net_conditions)}")'
                else:
                    # Fallback to generic condition
                    condition = "(condition \"A.inDiffPair('*')\")"
            else:
            ...
            clauses = [
                f'(layer "{layer_name}")',
                (f"(constraint track_width (opt {_mm_str(w_mm)}))"),
                (f"(constraint diff_pair_gap (opt {_mm_str(s_mm)}))"),
                condition,
            ]
            rule_name = f"diff_pair_Z{int(zdiff)}_{layer_name.replace('.', '_')}"
            rules.append(_emit_rule(rule_name, clauses))
    ...
    for dp in diff_pairs:
        ...
        try:
            ...
            if min_skew_mm is not None:
                ...
                clauses = [
                    f"(constraint skew (max {_mm_str(min_skew_mm)}))",
                    f"(condition \"A.inDiffPair('{base_name}')\")",
                ]
                rule_name = f"diff_pair_skew_{base_name.replace('/', '_')}"
                rules.append(_emit_rule(rule_name, clauses))
