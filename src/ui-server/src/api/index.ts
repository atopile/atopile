 ...
 *
 * Falls back to window.location.origin so that when served by the test runner
 * (or any other server on a dynamic port), the UI connects to the same origin.
 */
export const API_URL =
  win.__ATOPILE_API_URL__ ||
  import.meta.env.VITE_API_URL ||
  (typeof window !== 'undefined' ? window.location.origin : 'http://localhost:8501');
