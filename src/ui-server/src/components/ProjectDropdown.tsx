...
import { ChevronDown, Layers, Search, Check } from 'lucide-react';
  ...
  return (
    <div className="install-dropdown project-dropdown" ref={dropdownRef}>
      <button
        ...
        onClick={toggleOpen}
        title={selectedProject ? selectedProject.name : placeholder}
        disabled={disabled || !hasProjects}
      >
        <Layers size={12} />
        <span>{selectedProject?.name || (hasProjects ? (showAllOption ? 'All' : placeholder) : 'No projects')}</span>
      </button>
      <button
