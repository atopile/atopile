"""Execution/logging helpers for the rewrite-native agent service."""
...
from atopile.agent import AgentRunner, mediator
from atopile.agent.activity_summary import ActivitySummarizer
from atopile.agent.config import AgentConfig
from atopile.agent.execution_context import AgentExecutionContext
from atopile.agent.provider import OpenAIProvider
from atopile.agent.registry import ToolRegistry
from atopile.agent.runner import TraceCallback
...
def _ensure_agent_logs_db() -> None:
    ...
    if _agent_logs_db_initialized:
        return
    try:
        from atopile.model.sqlite import AgentLogs, AgentSessions, MessageLog
...
async def emit_agent_progress(
    *,
    ...
    if _progress_emitter is not None:
        await _progress_emitter(
            {
                "type": EVENT_AGENT_PROGRESS,
                **event_payload,
            }
        )
...
def handle_build_completed(payload: dict[str, Any] | None) -> None:
    ...
    try:
        ...
        with sessions_lock:
            ...
            if current and current.active_run_id == run_id:
                ...
                current.run_started_at = None
                current.activity_label = "Stopped"
        persist_sessions_state()
        return
    except Exception as exc:
        ...
        with sessions_lock:
            ...
            if current:
                ...
                finalize_run_error(current, run_id=run_id, error=str(exc))
    ...
    with sessions_lock:
        ...
        with runs_lock:
            ...
            if latest is not None:
                if (
                    latest.stop_requested
                    or latest.consumed_interrupt_messages
                    or latest.interrupt_messages
                ):
                    terminal_phase = PHASE_STOPPED
        ...
        finalize_run_success(
            ...
            terminal_label="Stopped" if terminal_phase == PHASE_STOPPED else "Ready",
        )
