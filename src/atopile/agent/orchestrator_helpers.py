...
if TYPE_CHECKING:
    from atopile.agent.runner import (
        ProgressCallback,
...
def _build_steering_inputs_for_model(
    messages: list[str],
    *,
    ...
    for i, message in enumerate(messages):
        ...
        tag = f" [message_id={mid}]" if mid else ""
        bullets.append(f"- {message}{tag}")
    bullet_text = "\n".join(bullets)
    return [
        {
            "role": "user",
            "content": (
                ...
                f"{bullet_text}\n\n"
                "If you have an active checklist and this guidance introduces "
                "new tasks, call `checklist_add_items` to append them with "
                '`source="steering"` and the corresponding `message_id`. '
                "Otherwise, call `message_acknowledge` for each message_id "
                "to confirm you've noted it."
            ),
        }
    ]
...
def _extract_output_phase(response: dict[str, Any]) -> str | None:
    """Extract the phase metadata from the last message output item.

    Returns ``"commentary"``, ``"final_answer"``, or ``None``.
    """
    output = response.get("output", [])
    if not isinstance(output, list):
        return None
...
def _worker_discovery_tool_names() -> set[str]:
    from atopile.agent.mediator_catalog import discovery_tool_names

    return discovery_tool_names()


def _worker_execution_tool_names() -> set[str]:
    from atopile.agent.mediator_catalog import execution_tool_names
...
def _build_function_call_outputs_for_model(
    *,
    ...
    # Never attach the PDF inline — it gets loaded into context and can
    # blow the window.  The file is already uploaded to OpenAI; the model
    # can reference it by file_id whenever it needs to.
            ...
            "content": [
                {
                    "type": "input_text",
                    "text": (
                        f"datasheet_read uploaded a datasheet PDF{size_text}. "
