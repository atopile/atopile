...
from atopile.dataclasses import OpenLayoutRequest
...
def _resolve_layout_file_for_tool(
    *,
    ...
    resolved_target = next(
        (item for item in project.targets if item.name == target), None
    )
    if resolved_target is None:
        known = ", ".join(sorted(item.name for item in project.targets))
        raise ValueError(f"Unknown build target '{target}'. Available: {known}")

    request = OpenLayoutRequest(
        project_root=str(project_root),
        target=resolved_target,
    )
    try:
        return builds_domain.resolve_layout_path(request).resolve()
