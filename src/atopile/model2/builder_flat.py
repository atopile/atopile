...
def build(obj: dm1.Object) -> Instance:
    """Build the model."""
    muck = Muck()
    return muck.visit_object((), obj)


class Muck:
    """Muck's job is to walk through the tree and build a flat model from it."""

    def visit_object(self, addr: Ref, obj: dm1.Object, instance: Optional[Instance] = None) -> Instance:
        """Visit an object."""
        if instance and obj in instance.origin.supers_bfs:
            return instance
        elif obj.supers_bfs:
            instance = self.visit_object(addr, obj.supers_bfs[0], instance)
        else:
        ...
        instance.children_from_classes.update(
            {ref[0]: self.visit_object(addr + ref, value) for ref, value in child_objects}
        )
