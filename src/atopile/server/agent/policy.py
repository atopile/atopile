...
def _anchor(line: int, text: str) -> str:
    return f"{line}:{compute_line_hash(line, text)}"
...
class TestHashlinePolicy:
    def test_format_hashline_content_shape(self) -> None:
        output = format_hashline_content(["alpha", "beta"], start_line=10)
        lines = output.splitlines()

        assert re.fullmatch(r"10:[0-9a-f]{4}\|alpha", lines[0])
        assert re.fullmatch(r"11:[0-9a-f]{4}\|beta", lines[1])

    def test_parse_line_anchor_tolerates_copied_suffix(self) -> None:
        assert parse_line_anchor("7:beef|value") == LineAnchor(
            line=7,
            hash="beef",
        )
        assert parse_line_anchor("7 : beef  value") == LineAnchor(
    ...
    def test_apply_hashline_set_line(self, tmp_path: Path) -> None:
    ...
    def test_apply_hashline_replace_range(self, tmp_path: Path) -> None:
        file_path = tmp_path / "main.ato"
        _write(file_path, "a\nb\nc\nd\n")
    ...
    def test_apply_hashline_insert_after(self, tmp_path: Path) -> None:
        file_path = tmp_path / "main.ato"
        _write(file_path, "a\nb\n")
    ...
    def test_apply_hashline_rejects_overlapping_operations(
        self, tmp_path: Path
    ) -> None:
        file_path = tmp_path / "main.ato"
        _write(file_path, "a\nb\nc\n")

        with pytest.raises(ScopeError, match="Overlapping edit spans"):
    ...
    def test_apply_hashline_rejects_noop_edits(self, tmp_path: Path) -> None:
        file_path = tmp_path / "main.ato"
        _write(file_path, "a\nb\n")

        with pytest.raises(ScopeError, match="No changes made"):
            apply_hashline_edits(
                tmp_path,
                "main.ato",
                [
    ...
    def test_apply_hashline_mismatch_includes_remap_hints(self, tmp_path: Path) -> None:
        file_path = tmp_path / "main.ato"
        _write(file_path, "a\nb\nc\n")
    ...
    def test_read_file_chunk_resolves_legacy_deps_path(self, tmp_path: Path) -> None:
        package_file = (
            tmp_path
            / ".ato"
            / "modules"
    ...
    def test_detect_datasheet_format_does_not_trust_pdf_suffix_for_html(self) -> None:
    ...
    def test_detect_datasheet_format_preserves_pdf_hint_for_non_html_bytes(
        self,
    ) -> None:
        detected = policy_datasheet._detect_datasheet_format(
            source_value="https://example.com/datasheet.pdf",
    ...
    def test_create_path_rejects_disallowed_extensions(self, tmp_path: Path) -> None:
        with pytest.raises(ScopeError, match="Only .ato, .md, .py"):
            create_path(tmp_path, "plans/notes.txt", kind="file", content="")

    def test_create_path_restricts_python_files_to_fabll_modules(
        self,
        tmp_path: Path,
    ) -> None:
        with pytest.raises(ScopeError, match="fabll modules"):
            create_path(tmp_path, "scripts/helper.py", kind="file")
    ...
    def test_read_datasheet_file_rejects_non_pdf_payload_from_pdf_url(
        ...
        with pytest.raises(ScopeError, match="not a valid PDF"):
            read_datasheet_file(
                tmp_path,
                url="https://example.com/datasheet.pdf",
            )

    def test__lcsc_wmsc_url_extracts_part_number(self) -> None:
        url = (
            "https://www.lcsc.com/datasheet/"
            "lcsc_datasheet_2304140030_STMicroelectronics-STM32G474RET6_C521608.pdf"
        )
        fallback = policy_datasheet._lcsc_wmsc_url(url)
        assert fallback == "https://wmsc.lcsc.com/wmsc/upload/file/pdf/v2/C521608.pdf"

    def test__lcsc_wmsc_url_ignores_non_lcsc_urls(self) -> None:
        assert (
            policy_datasheet._lcsc_wmsc_url("https://www.st.com/resource/doc.pdf")
            is None
        )
    ...
    def test_read_datasheet_bytes_from_url_falls_back_to_wmsc_pdf(
        ...
        raw, source_value, content_type = (
            policy_datasheet._read_datasheet_bytes_from_url(
