...
def resolve_project_root(project_root: str, ctx: AppContext) -> Path:
    ...
    if ctx.workspace_paths:
        ...
        if not any(root.is_relative_to(ws) or root == ws for ws in allowed):
            raise ScopeError("Project root is outside the current workspace scope")
...
def _extract_pdf_text(
    *,
    ...
    if start_page > total_pages:
        raise ScopeError(f"start_page {start_page} exceeds total pages ({total_pages})")
...
# Colocated tests moved from `test/server/agent/test_hashline_policy.py`.
try:
    import pytest
