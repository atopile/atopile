...
from atopile.server.domains.parts import serialize_component
...
from faebryk.libs.util import robustly_rm_dir, sanitize_filepath_part
...
def _serialize_part(component: Component) -> dict:
    """Serialize a Component with pretty-printed attributes."""
    return serialize_component(component, attributes_formatter=_pretty_attributes)
...
def handle_get_install_identifier(
    ...
    try:
        epart = download_easyeda_info(lcsc_id, get_model=False)
    except Exception as exc:
        raise errors.UserException(str(exc)) from exc

    manufacturer, partno = [sanitize_filepath_part(x) for x in epart.mfn_pn]
    if not manufacturer:
...
def handle_install_part_as_package(
    ...
    package_name = str(package_seed.get("identifier", "")).replace("_package", "")
    if not package_name:
        raise errors.UserException("Installed part did not return an identifier")
    ...
    packaged = handle_install_part(lcsc_id, str(package_root))
