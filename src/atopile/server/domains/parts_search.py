...
def handle_install_part(lcsc_id: str, project_root: str) -> dict:
    ...
    datasheet_path = lifecycle.datasheets.get_cached_path(epart.lcsc_id)

    return {
        "identifier": apart.identifier,
        "path": str(apart.path),
        "datasheet_path": (
            str(datasheet_path) if datasheet_path and datasheet_path.exists() else None
        ),
    }


def handle_get_cached_datasheet_path(lcsc_id: str, project_root: str) -> str | None:
    config.apply_options(None, working_dir=Path(project_root))
    path = PartLifecycle.singleton().datasheets.get_cached_path(lcsc_id)
