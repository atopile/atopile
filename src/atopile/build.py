...
for parent in iter_parents(":Root::vdiv.r_top.1"):
    print(parent)
...
groupby(get_footprint, filter(match_components, all_descendants(ROOT)))
...
#%%
def get_component_footprints() -> dict:
    """Get a libpart from the instance"""
    libparts = defaultdict(list)
    footprints = {}
    components = filter(match_components, all_descendants(ROOT))
...
lp = get_libpart_unique_components()
fp = get_component_footprints()
print(lp)
print(fp)
