...
class Visitor(AtoParserVisitor):
    ...
    @staticmethod
    def _extract_source(ctx: ParserRuleContext) -> ir.SourceChunk:
    ...
    def visitTypeReference(self, ctx: AtoParser.Type_referenceContext) -> ir.TypeRef:
        type_ref = ir.TypeRef(self.visitName(ctx.name()))
    ...
    ) -> ir.BlockDefinition.BlockType:
        return ir.BlockDefinition.BlockType(ctx.getText())

    def visitBlockdef(self, ctx: AtoParser.BlockdefContext) -> ir.BlockDefinition:
        block_type = self.visitBlocktype(ctx.blocktype())

        block_definition = ir.BlockDefinition(
        ...
        for child in ctx.block().getChildren():
            match node_or_nodes := self.visit(child):
                case ir.CompilerNode() | ir.SourceChunk():
                    scope.add(node_or_nodes)
                case Iterable():
                    for node in node_or_nodes:
                        # FIXME: assert is CompilerNode
                        if isinstance(node, ir.CompilerNode):
                            scope.add(node)
                case None:
                    pass
                case _:
    ...
    def visitFile_input(self, ctx: AtoParser.File_inputContext) -> ir.File:
        file = ir.File()
        ...
        for child in ctx.getChildren():
            match node_or_nodes := self.visit(child):
                ...
                case Iterable():
                    for node in node_or_nodes:
                        assert isinstance(node, ir.CompilerNode), (
                            "missing a visitor method"
                        )
                        scope.add(node)
                case None:
                    pass
    ...
    def visitPragma_stmt(self, ctx: AtoParser.Pragma_stmtContext) -> ir.PragmaStmt:
        pragma = ir.PragmaStmt(ctx.PRAGMA().getText().strip())
        pragma.add(self._extract_source(ctx))
        return pragma

    def visitImport_stmt(self, ctx: AtoParser.Import_stmtContext) -> ir.ImportStmt:
        if ctx.string():
            from_path = ir.ImportPath(self.visitString(ctx.string()))
        ...
        import_stmt = ir.ImportStmt(from_path, type_ref)
        import_stmt.add(self._extract_source(ctx))

        return import_stmt

    def visittString_stmt(self, ctx: AtoParser.String_stmtContext) -> ir.StringStmt:
        string = ir.String(self.visitString(ctx.string()))
        stmt = ir.StringStmt(string)
    ...
    def visitField_reference_part(
        ...
        # TODO: handle keys
        field_ref_part = ir.FieldRefPart(self.visitName(ctx.name()))
    ...
    def visitAssign_stmt(
    ...
    ) -> ir.AssignNewStmt | ir.AssignQuantityStmt | None:
        ...
        if field_ref := field_ref_or_decl.field_reference():
            ...
            if new_stmt_ctx := ctx.assignable().new_stmt():
                ...
                count = None  # TODO
                new_stmt = ir.AssignNewStmt(field_ref, type_ref, template, count)
                ...
                assign_quantity_stmt = ir.AssignQuantityStmt(field_ref, quantity)
    ...
    ) -> ir.Quantity | ir.BilateralQuantity | ir.BoundedQuantity:
        if ctx.quantity():
    ...
    def visitQuantity(self, ctx: AtoParser.QuantityContext) -> ir.Quantity:
    ...
    def visitBilateral_quantity(
        ...
        bilateral_quantity = ir.BilateralQuantity(
    ...
    def visitBilateral_tolerance(
        ...
        quantity = ir.Quantity(self.visitNumber_signless(ctx.number_signless()), unit)
        quantity.add(self._extract_source(ctx))
        return quantity

    def visitMif(self, ctx: AtoParser.MifContext) -> ir.FieldRef:
    ...
    def visitConnect_stmt(self, ctx: AtoParser.Connect_stmtContext) -> ir.ConnectStmt:
        left, right = [self.visitMif(c) for c in ctx.mif()]
        connect_stmt = ir.ConnectStmt(left, right)
        connect_stmt.add(self._extract_source(ctx))
        return connect_stmt

    def visitDirected_connect_stmt(self, ctx: AtoParser.Directed_connect_stmtContext):
        if ctx.SPERM():
            direction = ir.DirectedConnectStmt.Direction.RIGHT
        elif ctx.LSPERM():
            direction = ir.DirectedConnectStmt.Direction.LEFT
        else:
        ...
        directed_connect_stmt = ir.DirectedConnectStmt(left, right, direction)
        directed_connect_stmt.add(self._extract_source(ctx))
        return directed_connect_stmt


def build_file(source_file: Path) -> ir.File:
