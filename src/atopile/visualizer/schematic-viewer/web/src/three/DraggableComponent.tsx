...
export const DraggableComponent = memo(function DraggableComponent({
  ...
  useFrame(() => {
    ...
    // Apply rotation + mirror from committed position
  ...
  const handlePointerDown = useCallback(
    (e: any) => {
      ...
      // Read scoped position (positions are keyed by "pathKey:itemId")
      ...
      const onMove = (me: PointerEvent) => {
        ...
        if (!hasDragged) {
          hasDragged = true;
          // Write to shared ref + tell store we're dragging
        ...
        if (dragOffset.current) {
          ...
          // Write directly to shared ref — no React, no snap
      ...
      const onUp = () => {
        if (!hasDragged) {
          ...
          // endDrag commits liveDrag → store (with snap)
          endDrag();
        }
    ...
    [component.id, screenToWorld, startDrag, endDrag, selectComponent, gl],
  );
