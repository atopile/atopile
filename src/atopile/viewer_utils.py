...
def get_parent(addr: AddrStr, build_ctx: atopile.config.BuildContext) -> AddrStr:
    ...
    return get_relative_addr_str(
        get_parent_instance_addr(addr), build_ctx.project_context.project_path
    )
...
def get_blocks(
    addr, build_ctx: atopile.config.BuildContext
    ...
    for child in get_children(addr):
        if (
            match_modules(child)
            or match_components(child)
            or match_interfaces(child)
            or match_pins_and_signals(child)
        ):
            type = "module"
            ...
            block_dict[get_name(child)] = {
                ...
                "address": get_relative_addr_str(
                    child, build_ctx.project_context.project_path
                ),
            }
...
def process_links(addr: AddrStr) -> list[dict]:
    ...
    for link in links:
        ...
        source_block, source_port = split_list_at_n(
            get_current_depth(addr), split_addr(link.source.addr)
        )
        target_block, target_port = split_list_at_n(
            get_current_depth(addr), split_addr(link.target.addr)
        )

        _source = {
            "block": get_name(combine_addr(source_block)),
            "port": combine_addr(source_port),
        }
        _target = {
            "block": get_name(combine_addr(target_block)),
            "port": combine_addr(target_port),
        }
        link_list.append(
            {
                "source": _source,
...
def is_path_without_end_nodes(G, blocks, source, target):
    ...
    for block in blocks:
        if (
            block not in (source, target)
            and (
                blocks[block]["type"] == "module"
                or blocks[block]["type"] == "component"
            )
            and block in G_temp.nodes
        ):
    ...
    for source in blocks:
        if blocks[source]["type"] == "module" or blocks[source]["type"] == "component":
            for target in blocks:
                if (
                    blocks[target]["type"] == "module"
                    or blocks[target]["type"] == "component"
                ):
                    if source != target and is_path_without_end_nodes(
                        G, blocks, source, target
                    ):
