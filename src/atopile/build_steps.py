...
def generate_testpoints(
    ...
    if not app.has_trait(F.has_autogenerated_testpoints):
        return

    from faebryk.exporters.pcb.testpoints.testpoints import (
        decorate_nets_with_testpoints,
    )

    decorate_nets_with_testpoints(app)


@muster.register(
    "position-testpoints",
    dependencies=[generate_testpoints, prepare_nets],
)
def position_testpoints(
    app: Module, solver: Solver, pcb: F.PCB, log_context: LoggingStage
) -> None:
    if not app.has_trait(F.has_autogenerated_testpoints):
        return

    from faebryk.exporters.pcb.testpoints.testpoints import position_testpoints

    pcb.transformer.attach()
    position_testpoints(app)


@muster.register(
    "update-pcb",
    description="Updating PCB",
    dependencies=[post_solve_checks, position_testpoints],
)
def update_pcb(
    app: Module, solver: Solver, pcb: F.PCB, log_context: LoggingStage
) -> None:
    ...
    if config.build.hide_designators:
        for fp in GraphFunctions(app.get_graph()).nodes_of_type(F.Footprint):
            fp.add(F.has_hidden_designator())
    pcb.transformer.hide_hidden_designators()
