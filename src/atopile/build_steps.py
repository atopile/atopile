...
class MusterTarget:
    ...
    def __call__(self, ctx: BuildStepContext) -> None:
        ...
        ctx.stage = self.name
        if ctx.build_id:
            AtoLogger.set_active_stage(self.name)
        logger.debug(f"Starting build stage: {self.name}")
...
def post_instantiation_setup(ctx: BuildStepContext) -> None:
    ...
    F.DataBus.has_databus_specification.resolve_data_bus_specification_parameters(
        app.g, app.tg
    )
